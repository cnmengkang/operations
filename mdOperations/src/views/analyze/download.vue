<template>
    ...
    <div class="analysis" v-else>
        <div class="day" v-if="dayData.length > 0">
            <tableCharts title="日报" tableTitle="客户端日下载情况表" chartTitle="客户端日下载量" type="download" class="download_day"
                :chartData="dayData" keys="day_title" />
        </div>
        <div class="week" v-if="weekData.length > 0">
            <tableCharts tableTitle="客户端周下载情况表" chartTitle="客户端周下载量" type="download" title="周报" class="download_week"
                :chartData="weekData" keys="week_title" />
        </div>
        <div class="month" v-if="monthData.length > 0">
            <tableCharts tableTitle="客户端月下载情况表" chartTitle="客户端月下载量" type="download" title="月报" class="download_month"
                :chartData="monthData" keys="month_title" />
        </div>
        <div class="weekFunc" v-if="weekFunc.length > 0">
            <tableChartsLine title="周走势图表" tableTitle="客户端周下载情况表" chartTitle="客户端周下载量" :tlTitle="weekTitle"
                type="download" class="download_weekFunc" :chartData="weekFunc" />
        </div>
        <div class="monthFunc" v-if="monthFunc.length > 0">
            <tableChartsLine title="月走势图表" tableTitle="客户端月下载情况表" chartTitle="客户端月下载量" :tlTitle="monthTitle"
                type="download" class="download_monthFunc" :chartData="monthFunc" />
        </div>
    </div>
</template>
...
import { getDownloadAnalyze, type DateParams } from "@/api/aiAnalyze";
...
let getLoading = async () => {
    let result = await getDownloadAnalyze(params);
    ...
    processTableTitle("download", result)
